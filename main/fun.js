...
export const sortedQuests = () => {
  ...
  data.forEach((quest) => {
    if (setting.level === "all") {
      ...
      if (
        quest.difficulty === setting.level &&
        setting.category.includes(quest.category)
      ) {
        sortedData.push(quest);
      }
    }
...
export const createCard = (
  ...
  cardQuestDiv.addEventListener("click", (e) => {
    ...
    divDescription.style.display =
      divDescription.style.display === "none" ? "block" : "none";
  });
...
const addAch = (numTrueQuest, herf) => {
  const trueQuests = JSON.parse(
    localStorage.getItem("AboutTheUsers"),
  ).trueQuest;
  if (numTrueQuest === trueQuests) {
    ...
    switch (true) {
      case numTrueQuest === 1:
        p.textContent =
          "Вы выполнили одно задание! Вы получили бронзовую звезду";
        break;
      case numTrueQuest === 2:
        p.textContent =
          "Вы выполнили два задания! Вы получили серебрянную звезду";
        break;
      case numTrueQuest === 5:
      ...
      case numTrueQuest === 10:
        p.textContent =
          "Вы выполнили десять заданий! Вы полусили изумрудную звезду";
        break;
